...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // validate request-line (method, target, http-version)
            ...
            // validate if method is GET
            if (strcmp(requestLine[0], "GET") != 0) {
                error(405);
                continue;
            }

            // validate if first character of target is /
            if (requestLine[1][0] != '/') {
                error(501);
                continue;
            }
            
            // validate if target contains a "
            if (strchr(requestLine[1], '\"') != NULL) {
                error(400);
                continue;
            }
            
            // validate if http version is correct
            if (strcmp(requestLine[2], "HTTP/1.1") != 0) {
                error(505);
                continue;
            }
            
            // validate if absolute path contains a .
            char *requestTarget = requestLine[1];
            char *afterAbsPath = strstr(requestTarget, "?");
            int len;

            if (afterAbsPath)
                len = afterAbsPath - requestTarget;
            else
                len = strlen(requestTarget);
            
            char absolutePath[len];
            strncpy(absolutePath, requestTarget, len);
            absolutePath[len] = '\0';
            ...
            if (afterAbsPath != NULL) {
                if (strlen(afterAbsPath) > 1) {
                    query = afterAbsPath + 1;
                }
            }
                       
            // concatenate root and absolute-path
            char path[strlen(root) + strlen(query) + 1];
            strcpy(path, root);
...
const char* lookup(const char* extension)
{
    ...
    if (strcasecmp("html", extension))
        return "text/html";
    
    if (strcasecmp("gif", extension))
        return "image/gif";
    
    if (strcasecmp("ico", extension))
        return "image/x-icon";
        
    if (strcasecmp("jpg", extension))
        return "image/jpeg";
